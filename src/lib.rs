...
///
/// Returns the captured `print` / `?` output on success, or every
/// diagnostic encountered (one per pass, joined with blank lines) on
/// failure. Parser errors are *all* reported — the parser recovers past
/// them so a single broken statement doesn't hide the rest.
pub fn run(source: &str, name: &str) -> Result<String, String> {
    ...
    let (program, parse_diags) = parser::parse_recovering(&file, tokens);
    if !parse_diags.is_empty() {
        return Err(render_diags(&file, &parse_diags));
    }
...
fn render_diags(file: &SourceFile, diags: &[Diagnostic]) -> String {
    diags
        .iter()
        .map(|d| d.render(file))
        .collect::<Vec<_>>()
